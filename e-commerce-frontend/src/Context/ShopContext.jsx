...
const ShopContextProvider = (props) => {
  ...
  useEffect(() => {
    ...
    {
      fetch('https://v4mart-backend.onrender.com/getcart', {
      method: 'POST',
      headers: {
  ...
  const addToCart = (itemId) => {
    ...
    if(localStorage.getItem("auth-token"))
    {
      fetch('https://v4mart-backend.onrender.com/addtocart', {
      method: 'POST',
      headers: {
        Accept:'application/form-data',
        'auth-token':`${localStorage.getItem("auth-token")}`,
  ...
  const removeFromCart = (itemId) => {
    ...
    if(localStorage.getItem("auth-token"))
    {
      fetch('https://v4mart-backend.onrender.com/removefromcart', {
      method: 'POST',
      headers: {
        Accept:'application/form-data',
        'auth-token':`${localStorage.getItem("auth-token")}`,
